...
mod tests {
    ...
    async fn invalid_query_percent_encoding_does_not_reach_upstream() {
        ...
        let event = events.first().expect("one audit event should exist");
        assert_eq!(event["decision"], "denied");
        assert_eq!(event["error_class"], "invalid_percent_encoding");
        assert_eq!(event["path"], "/v1/models");
        assert_eq!(event["query"], "bad=%zz");
    }

    #[tokio::test]
    async fn invalid_query_backslash_does_not_reach_upstream() {
        let _guard = lock_gateway_test().await;
        let (upstream, recorder) = start_upstream().await;
        let gateway =
            GatewayProcess::spawn(&format!("http://{upstream}"), "GET:exact:/v1/models").await;
    ...
    #[tokio::test]
    async fn mixed_case_encoded_dot_segments_are_denied_and_audited() {
        let _guard = lock_gateway_test().await;
        let (upstream, recorder) = start_upstream().await;
        let gateway =
        ...
        assert_eq!(
            recorded_hits(&recorder).len(),
            0,
            "mixed-case encoded dot segment paths must not reach the upstream"
        );
        ...
        for (event, path) in events.iter().zip(paths) {
            ...
            assert_eq!(event["error_class"], "dot_segment", "path {path}");
