...
const MAX_SCENARIO_BODY_BYTES: usize = 8;

/// Maximum request headers generated by deterministic scenarios.
const MAX_SCENARIO_HEADERS: usize = 8;
...
impl ScenarioBody {
    ...
    fn new(bytes: Vec<u8>) -> Self {
        assert!(
            bytes.len() <= MAX_SCENARIO_BODY_BYTES,
            "scenario request body should be bounded"
        );
        Self { bytes }
    }
}
...
impl ScenarioHeaders {
    ...
    fn new(fields: Vec<(String, String)>) -> Self {
        assert!(
            fields.len() <= MAX_SCENARIO_HEADERS,
            "scenario request headers should be bounded"
        );
        for field in &fields {
            let name = &field.0;
            let value = &field.1;
            HeaderName::from_bytes(name.as_bytes()).expect("scenario header name should parse");
            HeaderValue::from_str(value).expect("scenario header value should parse");
        }
        Self { fields }
    }
}
...
impl ScenarioRequest {
    ...
    pub(super) fn new(
        ...
        Self {
            body: ScenarioBody::new(body),
            headers: ScenarioHeaders::new(headers),
            target,
        }
    }
