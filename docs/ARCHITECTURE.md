...
writes a `response_error` audit event, and fails closed. Post-start stream
aborts attempt to deliver the same harness-visible stream error after the
terminal audit attempt completes. Delivery is bounded, so a permanently
stalled harness can still observe a closed stream without the error frame; the
exact failure class is visible only in the audit event.
...
and a monotonic sequence. The sequence makes identities unique within one
gateway run. The random token makes identities from different gateway runs
collision-resistant under the operating system random source assumption; the
...
deterministic class sweep covers 36 reachable classes every run: three audit
outcomes (`Record`, `EventTooLarge`, and `FailFirst`) across six
non-disconnect classes, plus those three audit outcomes across the two
disconnect points and three response-starting upstream outcomes. The property
